#!/usr/bin/env python3
"""
Standalone test for WebRTC VAD with more debug prints.
"""
import webrtcvad
import numpy as np
...
def main():
    ...
    def audio_callback(indata, frames, time_info, status):
        nonlocal triggered, speech_start_time, silence_start_time, recorded_audio, stop_requested, callback_count, speech_frame_count, is_speech_count
        callback_count += 1
        if status:
        ...
        if is_speech:
            ...
            if not triggered:
                ...
                silence_start_time = None
            # Debug: print first few speech frames
            if speech_frame_count < 5:
                print(f"  Speech frame {speech_frame_count}: is_speech=True, audio_float shape={audio_float.shape}, first 5 samples: {audio_float[:5]}")
    ...
    if recorded_audio:
        ...
        print(f"Is speech True count: {is_speech_count}")
